...
if __name__ == "__main__":
    ...
    # Get the name of the dataset and model from the estimator path
    dataset_name, model_name = args.estimator.parts[-3:-1]

    # Open the dataset, if not already downloaded
    in_data_path = Path('datasets') / f'{dataset_name}.h5'
    if not in_data_path.exists():
        with open('datasets.yml') as fp:
            datasets = yaml.safe_load(fp)
            if args.dataset not in datasets:
                raise ValueError(f'No such dataset: {dataset_name}. See `datasets.yml` for available options')
            dataset_url = datasets[args.dataset]['url']

        my_logger.info(f'Downloading data for {dataset_name}')
    ...
    if model_name not in _models:
        raise ValueError(f'No such model: {model_name}. Options: {", ".join(_models.keys())}')
    model = _models[model_name]
    my_logger.info(f'Selected model: {model}')
