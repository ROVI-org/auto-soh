...
class SOCFitters():
    ...
    """
    def __init__(self,
                 style: Literal['interpolate', 'smooth', 'lsq'] = 'interpolate',
                 parameters: Dict = {}):
        self.style = style
    ...
    @property
    def style(self) -> Literal['interpolate', 'smooth', 'lsq']:
        return self._style
    ...
    def fit(self, soc: np.ndarray, targets: np.ndarray, **kwargs) -> Callable:
        ...
        """
        if self._style == 'interpolate':
            return make_interp_spline(x=soc, y=targets, **self.parameters.update(kwargs))
        elif self._style == 'smooth':
            return make_smoothing_spline(x=soc, y=targets, **self.parameters.update(kwargs))
        elif self._style == 'lsq':
            if 't' not in self._params.keys() and 't' not in kwargs.keys():
                raise ValueError('Please provide knots for LSQ Spline!')
            return make_lsq_spline(x=soc, y=targets, **self.parameters.update(kwargs))
        elif self._style == 'isotonic':
